...
low, high = df.abundance.quantile([0.25, 0.75])
...
for i in range(len(grid.optimizer_results_)):
    ...
    plt.savefig('images/grid/optimizer_results_multiclass_25-75_{}.png'.format(i+1))
    plt.close()
...
out_array.tofile('results/testR-multiclass_25-75_1.csv', sep=',')
#print()

print('Plotting...')
graph.plot_confusion_matrix_multi(y_pred=y_pred_cat, y_actual=y_test, title='Expression Classification', filename='images/confusion-matrix/CM-multiclass_25-75_1.png')

test_loss, test_auc, test_acc, test_precision, test_recall = best_model.model_.evaluate(X_test_seq, y_test_dummy)
print('25-75 Split Results:')
