...
pretrained_model = AutoModelForSequenceClassification.from_pretrained('./models/aminoBERT-multi-large_0/checkpoint-63665')
model = AutoModelForSequenceClassification.from_config(config)

#model = AutoModelForSequenceClassification.from_pretrained('./models/aminoBERT-multi-large_0/checkpoint-63665/')
...
training_args = TrainingArguments(
    output_dir='./models/aminoBERT_multi_reg_{}-pre-norm2'.format(SPECIES),
    learning_rate=1e-5,
...
with open('./results/aminoBERT_multi_reg_scores_{}-pre-norm2.txt'.format(SPECIES),'w') as data: 
    data.write(str(metrics))

with open('./results/aminoBERT_multi_reg_output_{}-pre-norm2.txt'.format(SPECIES),'w') as data:
    for val in logits:
        data.write(str(val)+'\n')

with open('./results/aminoBERT_multi_reg_labels_{}-pre-norm2.txt'.format(SPECIES),'w') as data:
    for val in labels:
        data.write(str(val)+'\n')
