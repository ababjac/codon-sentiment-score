...
#model = AutoModelForSequenceClassification.from_pretrained('./models/aminoBERT-binary-large_1/checkpoint-127330', num_labels=2)
model = AutoModelForSequenceClassification.from_config(config) #randomly initialize it

training_args = TrainingArguments(
    output_dir='./models/aminoBERT-binary-large_{}'.format(RUN),
    learning_rate=2e-6,
...
with open('./results/aminoBERT-binary-large_{}.txt'.format(RUN),'w') as data: 
