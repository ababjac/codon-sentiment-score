...
df = df[df['species'] == 'yeasts288c'] #train on only yeast sequences
...
model = BertRegresser.from_pretrained('./models/codonBERT-multi_1/checkpoint-127000', config=config)
#optimizer = torch.optim.Adam(params=model.parameters())

training_args = TrainingArguments(
    output_dir='./models/codonBERT-multi_reg_yeast'.format(RUN),
    learning_rate=1e-5,
...
with open('./results/codonBERT-multi_reg_yeast.txt'.format(RUN),'w') as data: 
