...
def create_model(lstm_units=1, neurons_dense1=5, neurons_dense2=3, dropout_rate=0.1, embedding_size=2, max_text_len=helpers.VOCAB_SIZE, learning_rate=0.5, output_neurons=1, output_activation='sigmoid', loss_function=tf.keras.losses.BinaryCrossentropy()):
    if output_neurons > 1:
        ...
        compute_metrics = [tf.keras.metrics.AUC(), tf.keras.metrics.Accuracy(), tf.keras.metrics.Precision(), tf.keras.metrics.Recall()]
